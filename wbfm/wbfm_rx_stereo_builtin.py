...
class wbfm_rx_stereo_builtin(gr.top_block):
    ...
    def __init__(self, centre_freq=int(100e6), ip_arg='localhost', port_arg=int(2e3), samp_rate=int(10e6), signal_freq=int(102.5e6)):
        ...
        self.blocks_multiply_const_vxx_0_0 = blocks.multiply_const_ff(0.75)
        ...
        self.connect((self.low_pass_filter_1_0, 0), (self.blocks_multiply_const_vxx_0_0, 0))
    ...
    def set_samp_rate(self, samp_rate):
        with self._lock:
            ...
            self.low_pass_filter_0.set_taps(firdes.low_pass(1, (int(self.samp_rate / self.decim)), 110e3, 20e3, window.WIN_HAMMING, 6.76))
    ...
    def set_audio_samp_rate(self, audio_samp_rate):
        with self._lock:
            ...
            self.low_pass_filter_1.set_taps(firdes.low_pass(1, self.audio_samp_rate, 15e3, 1e3, window.WIN_HAMMING, 6.76))
            self.low_pass_filter_1_0.set_taps(firdes.low_pass(1, self.audio_samp_rate, 15e3, 1e3, window.WIN_HAMMING, 6.76))
...
def main(top_block_cls=wbfm_rx_stereo_builtin, options=None):
    if options is None:
